...
export const protect = async (req, res, next) => {
    ...
    if (req.headers.authorization && req.headers.authorization.startsWith('Bearer')) {
        try {
            ...
            const users = await queryDatabase('SELECT user_id, full_name, email, role FROM users WHERE user_id = ?', [decoded.id]);
            ...
            return res.status(401).json({ error: 'Không được phép, token không hợp lệ hoặc đã hết hạn.' });
        }
    }
